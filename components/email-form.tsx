...
export function EmailForm({
  ...
  return (
    <div className="relative w-full">
      {/* Success message — CSS fade, always in DOM to avoid layout shift */}
      <div
        className={`flex items-center justify-start lg:justify-center gap-3 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 transition-all duration-300 ${
          isSuccess
      ...
      {/* Form — CSS fade out when success */}
      <form
        onSubmit={handleSubmit}
        className={`relative flex items-center w-full group transition-all duration-300 ${
          isSuccess ? 'opacity-0 -translate-y-2 pointer-events-none' : 'opacity-100 translate-y-0'
        }`}
        aria-hidden={isSuccess}
      >
        <input
      ...
      <p className="text-[10px] sm:text-[11px] text-neutral-800 dark:text-neutral-200 mt-4 text-left lg:text-center whitespace-nowrap tracking-tight font-medium">
        ...
        <Link href="/terms" className="underline hover:text-black dark:hover:text-white transition-colors">Terms</Link>
        {' & '}
        <Link href="/privacy" className="underline hover:text-black dark:hover:text-white transition-colors">Privacy Policy</Link>.
      </p>
    </div>
  );
}
